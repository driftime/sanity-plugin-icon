 ...
 */
export function normaliseTerms(value: string) {
  return value
    .toLowerCase()
    .replaceAll(/[\s-]+/gu, " ")
    .trim();
    ...
    return [
      {
        ...
        tags: iconTags.join(", "),
        node,
        terms: normaliseTerms([name, ...iconTags].join(" ")),
      },
    ];
  });
}

/** The library once asked for, held so every field in a Studio session shares the one request. */
let libraryRequest: Promise<LibraryIcon[]> | undefined = undefined;

/**
 * Reads the icon library, sharing a single request across every field opened in the session. The
 * library is a megabyte of data that never changes within a release, so asking twice only ever costs.
 *
 * @returns Every icon the library offers.
 */
